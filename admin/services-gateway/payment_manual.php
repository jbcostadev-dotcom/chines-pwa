...
class MultiGatewayProcessor
{
    ...
    public function processGreePay($config, $transacao_id, $valor, $chavepix, $cpf, $nome_real)
    {
        if (empty($config['client_id']) || empty($config['client_secret']) || empty($config['url'])) {
            return ['success' => false, 'message' => 'GreePay: Credenciais não configuradas'];
        }
    
        $api_url = rtrim($config['url'], '/');
        $this->debug[] = "GreePay: Iniciando processamento de saque (transação $transacao_id)";
        ...
        $this->debug[] = "GreePay: Tipo de chave: $pix_type, Chave formatada: $pix_key_formatada";
        ...
        $payload = [
            ...
            'webhook_url' => $GLOBALS['url_base'] . 'callbackpayment/greepay'
        ];
        ...
        logAction('GreePay: Enviando requisição de saque', [
        ...
        // Análise da resposta
        if (!$response['success']) {
            logAction('GreePay: Erro na comunicação', $response);
            return [
                'success' => false,
                'message' => 'GreePay: Falha de comunicação - ' . ($response['message'] ?? 'sem mensagem'),
                'response' => $response
            ];
        }
        ...
        if ($success_http && isset($data['ok']) && $data['ok'] === true) {
            ...
            $this->debug[] = "GreePay: Saque criado com sucesso - E2E: $e2e";
        }
    
        // Log completo
        logAction('GreePay: Resposta completa', [
            'http_code' => $http_code,
        ...
        if ($eh_sucesso) {
            return [
                ...
                'e2e' => $e2e,
                'message' => 'Saque processado via GreePay com sucesso',
                'response_data' => $data
            ];
        }
...
// Buscar credenciais dos gateways (ordem de prioridade: GreePay > AurenPay > BSPay > ExpfyPay)
...
// 1️⃣ Tentar GreePay primeiro (se disponível e preferido ou se for o prioritário)
if ($greepay_cred && (!$gateway_preferido || $gateway_preferido === 'greepay')) {
    ...
    if ($result['success']) {
        $gateway_usado = 'greepay';
    } else {
        $errors['greepay'] = $result['message'];
    }
}

...
#=====================================================#
# DATA CONFIG
// Funções removidas: data_pgclone, data_ppclone
...
function enviarRequest($url, $config)
{
    ...
    curl_setopt($ch, CURLOPT_IPRESOLVE, CURL_IPRESOLVE_V4); // Forca IPv4
...
function distribution($code) {
    return 'PlayFiver'; // Forçando todos os jogos a usarem PlayFiver
}
...
function pegarLinkJogoApiPlayFiver($provedor, $game, $email, $saldo)
{
    ...
    if (!$keys || empty(trim($keys['url'] ?? '')) || empty(trim($keys['agent_token'] ?? '')) || empty(trim($keys['agent_secret'] ?? '')) || empty(trim($keys['agent_code'] ?? ''))) {
        return [ 'gameURL' => null, 'error' => 'Configuração da PlayFiver ausente ou incompleta.' ];
    }
    ...
    if ($provedor === 'PGSOFT' || $provedor == 'PG') { $isOriginal = false; }
    
    // Normalizando a URL para não ter barra no final
    $baseUrl = rtrim($keys['url'], '/');
    ...
    // Enviando para v2/game_launch primeiro, que é o padrão da API PlayFiver
    $response = enviarRequest($baseUrl . '/api/v2/game_launch', $json_data);
    $data_response = json_decode($response, true);
    
    error_log("[PlayFiver] Response: " . $response);
    
    // Verifica se o lançamento foi bem-sucedido via v2
    if (isset($data_response['status']) && intval($data_response['status']) === 1) {
        // Tenta pegar a URL do jogo (pode ser launch_url ou gameURL)
        $game_url = $data_response['launch_url'] ?? $data_response['gameURL'] ?? null;
        ...
        if ($game_url) {
            ...
            error_log("[PlayFiver] Erro - URL do jogo não encontrada na resposta v2");
            $games = array(
                'gameURL' => null, 
        ...
        $fallbackResp = enviarRequest($baseUrl, $fallbackJson);
        ...
        if (isset($fallbackData['status']) && intval($fallbackData['status']) === 1) {
            ...
            if ($game_url) {
                ...
                $games = array('gameURL' => null, 'error' => 'URL não encontrada no servidor remoto.');
            }
        } else {
            $msg = $fallbackData['msg'] ?? $data_response['msg'] ?? 'Jogos em manutenção';
            $games = array('gameURL' => null, 'error' => "Falha no lançamento: $msg");
        }
    }

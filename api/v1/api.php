...
if ($path === '/api/frontend/trpc/deposit.create' || $path === '/api/frontend/trpc/recharge.create' || $path === '/api/frontend/trpc/pay.create') {
    ...
    error_log("----------------------------------------------------------------");
    error_log("API: Iniciando solicitacao de deposito (pay.create/deposit.create)");
    error_log("API: Usuario ID: " . $id . " | Nome: " . $nome);
    error_log("API: Valor (reais): " . $amount);
    error_log("API: payTypeSubListId: " . $payTypeSubListId);
    try {
        error_log("API: Chamando kaioox_qrcode...");
        $result = kaioox_qrcode($amount, $nome, $id, $comissao, $afiliado_id);
        error_log("API: Retorno de kaioox_qrcode: " . print_r($result, true));
    } catch (Exception $e) {
        error_log("API: Exception em kaioox_qrcode: " . $e->getMessage());
        $result = null;
    } catch (Error $e) {
        error_log("API: Fatal Error em kaioox_qrcode: " . $e->getMessage());
        $result = null;
    }

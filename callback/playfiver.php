...
function handleTransaction($request) {
    ...
    // ✅ CORREÇÃO: Calcular ganho líquido
...
switch ($type) {
    ...
    case 'Bet':
    case 'Win':
        $agentCode = $requestData['agent_code'] ?? $requestData['agentToken'] ?? $requestData['agent_token'] ?? '';
        $agentSecret = $requestData['agent_secret'] ?? $requestData['secretKey'] ?? '';
